...
// POST /api/favorites/articles/[id] - Добавить или удалить статью из избранного
export async function POST(request: NextRequest, { params }: FavoriteParams) {
  try {
    console.log(`[/api/favorites/articles] Начало обработки запроса для ID: ${params.id}`);
    ...
    if (!user || !user.id) {
      console.log('[/api/favorites/articles] Ошибка: пользователь не авторизован', { error, userId: user?.id });
    ...
    if (!id || typeof id !== 'string') {
      console.log('[/api/favorites/articles] Ошибка: некорректный ID статьи');
      return NextResponse.json(
    ...
    try {
      ...
      console.error('[/api/favorites/articles] Ошибка при парсинге JSON запроса:', error);
      return NextResponse.json(
    ...
    if (!action || (action !== 'add' && action !== 'remove')) {
      console.log(`[/api/favorites/articles] Ошибка: некорректное действие: ${action}`);
      return NextResponse.json(
    ...
    console.log(`[/api/favorites/articles] Обработка запроса: ${action} для статьи ${id} от пользователя ${user.id}`);
    ...
    if (!currentUser) {
      ...
      console.log(`[/api/favorites/articles] Пользователь ${user.id} не найден в базе данных, создаем пустую структуру избранного`);
      ...
      try {
        ...
        // Если действие - добавление, добавляем статью
        if (action === 'add') {
          userFavorites.articles.push(id);
          const updatedFavoritesJson = JSON.stringify(userFavorites);
          usersAdapter.update(user.id, { favorites: updatedFavoritesJson });
          console.log(`[/api/favorites/articles] Добавлена статья ${id} в избранное для пользователя ${user.id}`);
        }
        
        return NextResponse.json({
          success: true,
          message: action === 'add' ? 'Статья добавлена в избранное' : 'Операция выполнена',
          data: { 
        ...
        console.error('[/api/favorites/articles] Ошибка при создании структуры избранного:', error);
        return NextResponse.json(
    ...
    try {
      ...
      if (!article) {
        console.log(`[/api/favorites/articles] Статья ${id} не найдена в базе данных`);
        return NextResponse.json(
          { success: false, message: 'Статья не найдена' },
          { status: 404 }
        );
      }
      console.log(`[/api/favorites/articles] Статья ${id} найдена: ${article.title}`);
    } catch (error) {
      console.error(`[/api/favorites/articles] Ошибка при проверке существования статьи ${id}:`, error);
      // Продолжаем выполнение, даже если не смогли проверить статью
    }
    ...
    if (currentUser.favorites) {
      ...
      if (typeof currentUser.favorites === 'string') {
        try {
          ...
          console.log(`[/api/favorites/articles] Успешно распарсили JSON избранного для пользователя ${user.id}`);
        } catch (error) {
          console.error('[/api/favorites/articles] Ошибка при парсинге JSON избранного:', error);
      ...
      // Если избранное - объект, используем его
        ...
        console.log(`[/api/favorites/articles] Избранное уже является объектом для пользователя ${user.id}`);
      }
    }
    ...
    if (action === 'add') {
      // Проверяем, не добавлена ли статья уже в избранное
      if (!userFavorites.articles.includes(id)) {
        userFavorites.articles.push(id);
        statusChanged = true;
        console.log(`[/api/favorites/articles] Добавлена статья ${id} в избранное для пользователя ${user.id}`);
      } else {
        console.log(`[/api/favorites/articles] Статья ${id} уже в избранном у пользователя ${user.id}`);
      }
    } else {
      // Удаляем статью из избранного
      const initialLength = userFavorites.articles.length;
      userFavorites.articles = userFavorites.articles.filter(
        articleId => articleId !== id
      );
      statusChanged = initialLength !== userFavorites.articles.length;
      
      if (statusChanged) {
        console.log(`[/api/favorites/articles] Удалена статья ${id} из избранного для пользователя ${user.id}`);
      } else {
        console.log(`[/api/favorites/articles] Статья ${id} не была найдена в избранном пользователя ${user.id}`);
      }
    }
    ...
    try {
      usersAdapter.update(user.id, { favorites: favoritesJson });
      console.log(`[/api/favorites/articles] Избранное обновлено для пользователя ${user.id}, статей: ${userFavorites.articles.length}`);
    } catch (error) {
      console.error('[/api/favorites/articles] Ошибка при обновлении избранного в базе данных:', error);
      return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      message: action === 'add' ? 'Статья добавлена в избранное' : 'Статья удалена из избранного',
      data: { 
        isFavorite: action === 'add',
        favorites: userFavorites,
        counts: counts
    ...
    console.error('[/api/favorites/articles] Ошибка при обновлении избранного:', error);
    return NextResponse.json(
